export { ReactComponent as LogoWhite } from "./logo-white.svg";
export { ReactComponent as LogogBlack } from "./logo-black.svg";
export { ReactComponent as LogoWhiteBig } from "./logo_white_big.svg";
export { ReactComponent as LogoFooter } from "./LogoFooter.svg";

export { ReactComponent as Home } from "./home.svg";
export { ReactComponent as GiftCard } from "./gift.svg";
export { ReactComponent as Contact } from "./contact.svg";
export { ReactComponent as Coin } from "./coin.svg";
export { ReactComponent as RateIcon } from "./rate.svg";
export { ReactComponent as Transactions } from "./transactions.svg";
export { ReactComponent as Wallet } from "./wallet.svg";

export { ReactComponent as Bell } from "./bell.svg";
export { ReactComponent as CaretDown } from "./caret-down.svg";
export { ReactComponent as Profile } from "./profile.svg";
export { ReactComponent as Power } from "./power.svg";
export { ReactComponent as Wave } from "./wave.svg";
export { ReactComponent as ArrowRight } from "./arrow-right.svg";
export { ReactComponent as ArrowLeft } from "./arrow-left.svg";
export { ReactComponent as SearchIcon } from "./search.svg";

export { ReactComponent as FastTrade } from "./fast-trade.svg";
export { ReactComponent as WalletCarrdBG } from "./wallet-card-bg.svg";

export { ReactComponent as BitcoinInput } from "./bitcoin-input.svg";
export { ReactComponent as NairaInput } from "./naira-input.svg";

export { ReactComponent as BtcAction } from "./btc-action.svg";
export { ReactComponent as NairaAction } from "./naira-action.svg";
export { ReactComponent as WithdrawalAction } from "./withdraw-action.svg";

export { ReactComponent as TableSort } from "./table-sort.svg";

export { ReactComponent as TransactionIcon } from "./transaction-icon.svg";
export { ReactComponent as Eye } from "./eye.svg";

export { ReactComponent as Edit } from "./Edit.svg";
export { ReactComponent as Warning } from "./warning.svg";

export { ReactComponent as CardSteam } from "./card-steam.svg";
export { ReactComponent as CardAmazon } from "./card-amazon.svg";
export { ReactComponent as CardApple } from "./card-apple.svg";
export { ReactComponent as CardItunes } from "./card-itunes.svg";

export { ReactComponent as UsdIcon } from "./usd-icon.svg";
export { ReactComponent as CardTypePhysical } from "./card-type-physical.svg";

export { ReactComponent as UploadIcon } from "./upload.svg";
export { ReactComponent as QRIcon } from "./qr-icon.svg";
// export { ReactComponent as Rocket } from "./rocket.svg";

export { ReactComponent as AppleIcon } from "./apple-icon.svg";
export { ReactComponent as AndroidIcon } from "./android.svg";

export { ReactComponent as Hero } from "./hero.svg";
export { ReactComponent as Rocket } from "./rocket.svg";

export { ReactComponent as MidRocket } from "./mid-rocket.svg";
export { ReactComponent as MidChart } from "./mid-chart.svg";
export { ReactComponent as MidMoney } from "./mid-money.svg";

export { ReactComponent as MidCards } from "./cards.svg";
export { ReactComponent as MidPhone } from "./hand-phone.svg";

export { ReactComponent as Exchange } from "./exchange.svg";
export { ReactComponent as WorldSmall } from "./worldsmall.svg";
export { ReactComponent as StarSmall } from "./midstart.svg";
export { ReactComponent as RocketTiny } from "./rocketTiny.svg";
export { ReactComponent as WorldBig } from "./worldBig.svg";

export { default as AUD } from "./AUD.svg";
export { default as AUT } from "./AUT.svg";
export { default as BEL } from "./BEL.svg";
export { default as BR } from "./BR.svg";
export { default as CAD } from "./CAD.svg";
export { default as DMK } from "./DMK.svg";
export { default as ESP } from "./ESP.svg";
export { default as EUR } from "./EUR.svg";
export { default as FIN } from "./FIN.svg";
export { default as FRA } from "./FRA.svg";
export { default as GBP } from "./GBP.svg";
export { default as GER } from "./GER.svg";
export { default as GRC } from "./GRC.svg";
export { default as IRL } from "./IRL.svg";
export { default as ITL } from "./ITL.svg";
export { default as JPY } from "./JPY.svg";
export { default as MXN } from "./MXN.svg";
export { default as NOK } from "./NOK.svg";
export { default as PRT } from "./PRT.svg";
export { default as RUB } from "./RUB.svg";
export { default as SWE } from "./SWE.svg";
export { default as USD } from "./USD.svg";

export { ReactComponent as Cancel } from "./cancel.svg";

export { ReactComponent as TransactionIconBig } from "./transaction-icon-med-pend.svg";
export { ReactComponent as TransactionIconMed } from "./transaction-icon-med.svg";
export { ReactComponent as TransactionIconSuc } from "./transaction-icon-med-suc.svg";

export { ReactComponent as Physical } from "./physical.svg";
export { ReactComponent as Barcode } from "./bar-code.svg";
export { ReactComponent as Copy } from "./copy.svg";
export { ReactComponent as Lock } from "./lock.svg";
